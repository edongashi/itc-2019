...
module Solution =
    let initial problem = problem |> Problem.initialSolution

    let withTime (time: int) (cls: int) (solution: Solution) = solution.WithTime(cls, time)

    let withRoom (room: int) (cls: int) (solution: Solution) = solution.WithRoom(cls, room)

    let withEnrollment (chain: int) (student: int) (solution: Solution) = solution.WithEnrollment(student, chain)

    let private constraints (solution: Solution) =
        solution.HardConstraintStates() |> Seq.toArray
    ...
    let classPenaltiesHard (solution: Solution) =
        ...
        let timePenalties =
            ...
            |> Seq.choose (fun pair ->
                ...
                if penalty > 0 then
                    Some(PenalizedTimeVariable(cls, solution.GetTimeIndex cls, penalty))
                else
                    None)
        ...
        let roomPenalties =
            ...
            |> Seq.choose (fun pair ->
                ...
                if penalty > 0 then
                    Some(PenalizedRoomVariable(cls, solution.GetRoomIndex cls, penalty))
                else
                    None)
    ...
    let classPenaltiesSoft (solution: Solution) =
        ...
        let timePenalties =
            ...
            |> Seq.choose (fun var ->
                ...
                if penalty > 0 then
                    Some(PenalizedTimeVariable(cls, classTime, penalty))
                else
                    None)
        ...
        let roomPenalties =
            ...
            |> Seq.choose (fun var ->
                ...
                if classData.PossibleRooms.Length > 0 && penalty > 0 then
                    Some(PenalizedRoomVariable(cls, classRoom, penalty))
                else
                    None)
    ...
    let fitnessProportionate (rnd: float) vars =
        let sum =
            float (vars |> List.sumBy variablePenalty)
    ...
    let penalize (increment: float) (rnd: float) (penalties: ClassPenalties []) (solution: Solution) =
        ...
        let list =
            if solution.HardPenalty > 0 then
                classPenaltiesHard solution
            else
                classPenaltiesSoft solution

        let penalizeClass penalties variable =
            let cls = variableClass variable

            penalties
            |> set cls (fun clsPenalties ->
                match variable with
                | PenalizedTimeVariable (_, index, _) ->
                    { clsPenalties with Times = set index (fun x -> x + increment) clsPenalties.Times }
                | PenalizedRoomVariable (_, index, _) ->
                    { clsPenalties with Rooms = set index (fun x -> x + increment) clsPenalties.Rooms })
    ...
    let stats (solution: Solution) =
        ...
        {| HardPenalty = solution.HardPenalty
           ...
           TimePenaltyAverage =
            (solution.TimePenalty() |> float)
            / (float problem.TimeVariables.Length)
           TimePenaltyMax = solution.TimePenaltyMax()
           RoomPenalty = solution.RoomPenalty()
           RoomPenaltyMin = solution.RoomPenaltyMin()
           RoomPenaltyAverage =
            (solution.RoomPenalty() |> float)
            / (float problem.RoomVariables.Length)
