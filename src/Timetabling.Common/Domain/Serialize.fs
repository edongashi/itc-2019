...
module Serialize =
    ...
    let rec private serializeRec content =
        match content with
        | Element (name, content) -> XElement(xname name, content |> List.map serializeRec |> Array.ofList) :> obj
        | Attribute (name, value) -> XAttribute(xname name, value) :> obj
    ...
    let element name content = Element(name, content)

    let attr name value = Attribute(name, value)

    let toXml content = content |> serializeRec :?> XElement

    let solution info seed (solution: SolutionModel) =
        let serializeStudent (StudentId id) = element "student" [ attr "id" id ]
