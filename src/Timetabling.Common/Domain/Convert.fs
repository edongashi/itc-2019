...
module IdMapping =
    let private resolve (map: IdMapping -> IdMap) fid mapping item = mapping |> map |> Map.find (fid item)
    ...
    let resolveRoomId =
        resolve (fun m -> m.Rooms) (fun (RoomId id) -> id)

    let resolveCourse = resolve (fun m -> m.Courses) Course.id

    let resolveCourseId =
        resolve (fun m -> m.Courses) (fun (CourseId id) -> id)

    let resolveConfig =
        resolve (fun m -> m.Configurations) Config.id

    let resolveSubpart = resolve (fun m -> m.Subparts) Subpart.id
    let resolveClass = resolve (fun m -> m.Classes) Class.id

    let resolveClassId =
        resolve (fun m -> m.Classes) (fun (ClassId id) -> id)
    ...
    let fromProblem (problem: ProblemModel) =
        let rooms = problem.Rooms |> forwardMap Room.id
        let courses = problem.Courses |> forwardMap Course.id
        ...
        let students =
            problem.Students |> forwardMap Student.id
...
module Convert =
    ...
    let private mapToArray f s = s |> Seq.map f |> Seq.toArray

    let private mapiToArray f s = s |> Seq.mapi f |> Seq.toArray
    ...
    let fromUnavailableTime (UnavailableTime time) = fromTime time
    ...
    let fromRoom ids (room: Room) =
        ...
        Timetabling.Internal.Room(
            ...
            capacity,
            room.UnavailableTimes
            |> mapToArray fromUnavailableTime,
            room.TravelTimes
            |> mapToArray (fromTravelTime ids)
        )

    let fromRoomAssignmentDetails ids (r: RoomAssignmentDetails) =
        let (RoomAssignmentPenalty penalty) = r.Penalty
        Timetabling.Internal.RoomAssignment(resolveRoomId ids r.RoomId, penalty)
    ...
    let fromTimeAssignment (timeAssignment: TimeAssignment) =
        ...
        Timetabling.Internal.ScheduleAssignment(time.Weeks, time.Days, time.Start, time.Length, penalty)
    ...
    let fromClass ids (cls: Class) =
        ...
        Timetabling.Internal.Class(
            ...
            cls.Parent
            |> Option.map (resolveClassId ids)
            |> Option.defaultValue -1,
            limit,
            cls.PossibleRooms
            |> listOfRoomAssignment
            |> mapToArray (fromRoomAssignmentDetails ids),
            cls.PossibleTimes |> mapToArray fromTimeAssignment
        )

    let fromSubpart ids (subpart: Subpart) =
        Timetabling.Internal.Subpart(resolveSubpart ids subpart, subpart.Classes |> mapToArray (fromClass ids))
    ...
    let fromCourse ids (course: Course) =
        Timetabling.Internal.Course(
            ...
            course.Configurations
            |> mapToArray (fromConfiguration ids)
        )
    ...
    let fromStudent ids (student: Student) =
        Timetabling.Internal.Student(
            ...
            student.Courses
            |> mapToArray (resolveCourseId ids)
        )
    ...
    let fromDistribution ids index (distribution: Distribution) : IConstraint =
        let required, penalty =
            match distribution.Requirement with
            | Required -> true, 0
    ...
    let fromProblem (problem: ProblemModel) =
        ...
        (ids,
         Timetabling.Internal.Problem(
             ...
             |> mapiToArray (fromDistribution ids)
         ))
    ...
    let fromSolution (problem: Problem) (solution: SolutionModel) =
        ...
        let roomId room =
            room
            |> Option.map (fun (RoomId id) -> ids.Rooms.[id])

        let classId (ClassId id) = ids.Classes.[id]

        let studentId (StudentId id) = ids.Students.[id]
        ...
        let folder (s: Internal.Solution) (cls: SolutionClass) =
            ...
            let schedule =
                classData.FindSchedule(start, days, weeks)

            try
                s'.WithTime(classId, schedule)
            with
            | _ -> s'
...
module Problem =
    ...
    let parse xml = Parse.problem xml |> Result.map wrap
