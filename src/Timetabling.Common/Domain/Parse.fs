...
module private XmlUtils =
    ...
    let children name (xml: XElement) = nameOfString name |> xml.Elements
    ...
    let havingName name (xml: XElement) =
        if tagName xml = name then
            Ok xml
        else
            Error { Element = xml; Error = InvalidName }

    let ensureName name xml =
        xml |> havingName name |> Result.discard

    let traverseChildren name f xml =
        xml |> children name |> Result.traverseSeq f
...
module Parse =
    ...
    let unavailable xml =
        xml
        |> havingName "unavailable"
        >>= schedule UnavailableTime
    ...
    let classDetails xml =
        ...
        result {
            ...
            let parent =
                xml |> tryNumAttr "parent" |> Option.map ClassId
    ...
    let subpart xml =
        ...
        result {
            ...
            return { Id = SubpartId id; Classes = classes }
        }
    ...
    let distributions xml =
        ...
        result {
            ...
            return
                distributions
                |> List.filter (fun d -> List.length d.Classes > 1)
        }
    ...
    let student xml =
        ...
        result {
            ...
            return { Id = StudentId id; Courses = courses }
        }
